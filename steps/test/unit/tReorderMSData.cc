...
void test1(std::string msPath, std::string msOutPath) {
    ...
    std::shared_ptr<dp3::steps::InputStep> input_step = std::move(dp3::steps::InputStep::CreateReader(parset));
    auto reorderStep = std::make_shared<dp3::steps::Reorder>(parset, "");
    auto nullStep = std::make_shared<dp3::steps::NullStep>();
    
    input_step->setFieldsToRead(Step::kDataField | Step::kFlagsField | Step::kWeightsField | Step::kUvwField);
...
BOOST_AUTO_TEST_CASE(testReorder1) { 
    ...
    // Data File check
    BOOST_CHECK(compareBinaries("../resources/midbands/refTmps/reorderTestVis.tmp", 
        "./testOut.ms-part0000-I-b0.tmp"));

    // Weight File check
    BOOST_CHECK(compareBinaries("../resources/midbands/refTmps/reorderTestWeights.tmp", 
